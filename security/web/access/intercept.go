...
import (
	...
	"github.com/chinmobi/ginmod/security/consts"
...
// Get the AccessDecisionAgent's gin.HandlerFunc
...
// Create access decision gin.HandlerFunc for controller's gin.HandlerFunc
...
func (agent *AccessDecisionAgent) doDecide(c *gin.Context) bool {
	if err := agent.interceptor.decide(c); err != nil {
		agent.interceptor.deniedHandler.OnAccessDenied(c, NewAccessDeniedErr(err))
		return false
	}
	return true
}
